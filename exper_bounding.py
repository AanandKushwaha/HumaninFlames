...
cap=cv2.VideoCapture('thehero2.avi')
while True:
    ret,frame=cap.read()    
    ...
    #columns = len(img1[:,:,0])
    blank=f_f
    blank[np.where((blank != [0,6,0]).all(axis = 2))] = [128,0,128]
    ...
    #The current scale of the image 
        ...
        if sc.any() < 0.5:
           ...
           clone=cv2.putText(clone,"Fire not detected ", (15, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1,lineType=cv2.LINE_AA)
            ...
            clone=cv2.putText(clone,"Fire detected ", (15, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1,lineType=cv2.LINE_AA)
    ...
    #print ("shape, ", pick.shape)
        ...
        frame=imutils.resize(frame, width = min(400, im.shape[1]))
        im=cv2.add(im, frame)
        cv2.imshow('before NMS',im)
        clone=cv2.add(clone,frame)
